...
if (isset($_POST['login'])) {
    ...
    $stmt = mysqli_prepare($db, "SELECT * FROM tbl_cleared WHERE student_id = ?");
    ...
    if ($row && password_verify($password, $row['password'])) {
        $_SESSION['student_id'] = $row['student_id']; // Kailangan to para ma-verify yung username
        $_SESSION['student_name'] = $row['last_name'] . ', ' . $row['first_name']; // Ito yung mag didisplay sa Dashboard (ex. Welcome Administrator)
